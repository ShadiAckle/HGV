...
export const DATA_DOMAIN_MODELS: DataDomainModel[] = [
  {
    ...
    tables: [
      ...
      },
      {
        ...
        layer: 'dimension',
        domain: 'Marketing',
        purpose: 'ICM plan catalog — plan_id, plan_name, icm_role (Varicent-style).',
        grain: 'One row per plan_id',
        seededBy: '18a_seed_icm_plan_marketing.sql',
        apiEndpoints: ['GET /api/comp/metadata'],
        uiSurfaces: ['Rep picker plan context'],
        columns: [
          { name: 'plan_id', type: 'STRING', key: 'PK' },
          { name: 'plan_name', type: 'STRING' },
          { name: 'icm_role', type: 'STRING', note: 'marketing | marketing_manager | marketing_director' },
        ],
      },
      {
        id: 'fact_plan',
        ...
        layer: 'fact',
        domain: 'Marketing',
        purpose: 'Plan components (METRIC) and qualified-tour tier ladder (TIER) — drives What\'s next hero.',
        grain: 'plan_id + component + period_id',
        seededBy: '18a_seed_icm_plan_marketing.sql',
        apiEndpoints: [],
        uiSurfaces: ['My Compensation — What\'s next tier'],
        columns: [
          { name: 'plan_id', type: 'STRING', key: 'FK' },
          { name: 'component', type: 'STRING' },
          { name: 'component_type', type: 'STRING', note: 'METRIC | TIER' },
          { name: 'tier_seq', type: 'INT' },
          { name: 'min_qualified_tours', type: 'INT' },
          { name: 'payout_per_unit', type: 'DECIMAL' },
      ...
      },
      {
        ...
        layer: 'dimension',
        domain: 'Marketing',
        purpose: 'Payee registry — maps payee_id to employee_id and name (synced from dim_marketing_rep).',
        grain: 'One row per payee_id',
        seededBy: '01_MATERIALIZE_ALL_TABLES.sql Step 6.5',
        apiEndpoints: ['GET /api/comp/metadata'],
        uiSurfaces: ['Rep picker'],
        columns: [
          { name: 'payee_id', type: 'STRING', key: 'PK', note: 'Same as rep_id' },
      ...
      {
        id: 'fact_payee_plan',
        name: 'fact_payee_plan',
        layer: 'fact',
        domain: 'Marketing',
        purpose: 'Payee-to-plan assignment by period and icm_role — authoritative for marketing plan_id on rollup.',
        grain: 'payee_id + period_id',
        seededBy: '01_MATERIALIZE_ALL_TABLES.sql Step 6.5',
        apiEndpoints: ['GET /api/comp/metadata'],
        uiSurfaces: ['My Compensation header', 'Rep picker plan_id'],
        columns: [
          { name: 'payee_id', type: 'STRING', key: 'FK' },
          { name: 'plan_id', type: 'STRING', key: 'FK', note: 'Join to dim_plan' },
          { name: 'icm_role', type: 'STRING' },
          { name: 'period_id', type: 'STRING', key: 'FK' },
        ],
      },
      {
    ...
    joins: [
      ...
      { from: 'fact_payee_plan', to: 'dim_plan', on: 'plan_id', cardinality: 'N:1' },
      { from: 'fact_payee_plan', to: 'dim_payee', on: 'payee_id', cardinality: 'N:1' },
      { from: 'fact_plan', to: 'dim_plan', on: 'plan_id', cardinality: 'N:1' },
      { from: 'fact_marketing_rep_period', to: 'fact_payee_plan', on: 'rep_id, period_id', cardinality: 'N:1' },
